...
Proof.
  ...
  intros i. destruct l as [|a l']. simpl. done. simpl. case_eq (i < a); intros H; simpl; apply f_equal; apply slide_hold_length.
Qed.

Compute slide 1 [5;3;4;2].
Compute slide 3 [1;5;4;2].

Program Fixpoint sort' (l1:list nat) (l2:list nat) {measure (length l2)} : (list nat) * (list nat):=
  match l2 with
  | [] => (l1, [])
  (* | h :: t => (fun p1 => (fun p2 => sort' ((snd p1) ++ [fst p2]) (snd p2)) (slide (fst p1) t)) (slide h l1) *)
  | h :: t => sort' ((snd (slide h l1)) ++ [fst (slide (fst (slide h l1)) t)]) (snd (slide (fst (slide h l1)) t))
  end.
Obligation 1.
...
Definition sort (l:list nat) :=
  (fun p => fst p ++ snd p) (sort' [] l).

Compute sort [3;1;4;1;5;9;2;6;5;3;5].
Compute sort [1;2;3;4;5;6;7].
...
Qed.
(* 
Lemma uo : forall n l1 a l2, sorted (n :: l1 ++ a :: l2) -> sorted ((n :: l1) ++ l2).
Proof.
  intros n. induction l1. intros a l2. simpl. apply cons_cons_sorted.
  intros b l3. simpl. intros S. inversion S.
  inversion H3. apply cons_sorted. *)
(* 
Lemma append_sorted : forall l1 l2, sorted (l1 ++ l2) -> sorted l1 /\ sorted l2.
Proof.
  fix append_sorted 1.
  destruct l1; simpl. intros l2 S. apply conj. apply sorted_nil. apply S.
  intros l2 S.  elim S.
  
  induction l2; simpl. done. intros S. apply conj.
  apply sorted_nil. apply S. rewrite app_nil_r. intros S. apply conj.
  apply S. apply sorted_nil. intros S. apply conj.
   intros l2 S. apply conj.

  induction l1, l2; simpl; intros S. done.
  apply conj. apply sorted_nil. apply S.  apply conj.
  by rewrite app_nil_r in S. apply sorted_nil. apply conj.
   
  assert (l1 ++ [] = l1). by rewrite app_nil_r. 
  apply S.
  assert (l1 ++ [::])
  Check ((a :: l1) ++ [::]).  *)
...
Lemma slide_cons : forall a l, sorted (a :: l) -> a :: l = (snd (slide a l)) ++ [fst (slide a l)].
Proof.
  fix slide_cons 2.
  ...
  pose (slide_cons _ _ S2) as e. assert (h :: (snd (slide a t)) ++  [(fst (slide a t))] = h :: ((snd (slide a t)) ++ [fst (slide a t)])).
...
Lemma leq_cons_sorted : forall a h l, sorted (h :: l) -> a <= h -> sorted (a :: h :: l).
Proof.
...
Lemma leq_all_cons_sorted : forall h a t, sorted (h :: t) -> h <= a -> 
  sorted (snd (slide a t) ++ [fst (slide a t)]) -> sorted (h :: snd (slide a t) ++ [fst (slide a t)]).
Proof.
...
Proof.
  ...
  apply sorted_cons. apply ltnW. apply H. apply S. apply (cons_cons_sorted _ _ _ H0).
  apply ltnW, H. apply slide_sorted'. apply (cons_sorted _ _ S).
  apply leq_all_cons_sorted. apply S. move: H. rewrite ltnNge. move/eqP/eqP => H.
  apply H. apply slide_sorted'. apply (cons_sorted _ _ S).
Qed.


(* Lemma slide_front : forall h t, sorted (h :: t) -> 
 *)

Lemma app_cons_comm : forall (l1:list nat) n l2, l1 ++ n :: l2 = (l1 ++ [n]) ++ l2.
Proof.
  induction l1. intros n l2. done.
  intros n l2. rewrite -app_comm_cons -app_comm_cons -app_comm_cons. apply f_equal.
  apply IHl1.
Qed.
...
Lemma insert_sorted : forall a l, sorted l -> sorted (snd (slide a l)).
Proof. intros a l S; apply (app_sorted _ _ (slide_sorted' _ _ S)). Qed.
